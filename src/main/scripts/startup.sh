...
#APOLLO_APPID=hbcfc-eureka
#APOLLO_ENV=DEV
#APOLLO_DEVMETA=http://192.168.251.151:9001
...
#export JAVA_OPTS="$JAVA_OPTS -Dapp.id=$APOLLO_APPID -Denv=$APOLLO_ENV -Ddev_meta=$APOLLO_DEVMETA"
export JAVA_OPTS="$JAVA_OPTS -Dspring.profiles.active=$1"
...
until [[ (( counter -ge max_counter )) || "$(curl -X GET --silent --connect-timeout 1 --max-time 2 --head $SERVER_URL | grep "Content-Type:")" != "" ]];
do
    printf "."
    counter+=1
    sleep 5
